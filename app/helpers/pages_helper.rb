  ...
  #-------------------------------------------------------------------------------------------------------
    ...
    def admin_controls(elem, options= {})  
      ...
      if first
        ...
        up = link_to '', up_page_path(elem), :title=>"#{t('page.move_up')} #{elem.title}", :class=>'button up'
      end
      ...
      if last
        ...
        down=   link_to '', down_page_path(elem), :title=>"#{t('page.move_down')} #{elem.title}", :class=>'button down'
      end

      edit=   link_to '', edit_page_path(elem),             :title=>"#{t('page.edit')} #{elem.title}", :class=>'button edit'
      new=    link_to '', new_page_path(:parent_id=>elem),  :title=>"#{t('page.create_child')} #{elem.title}", :class=>'button new'
      
      # Если дочерние элементы отсутствуют
      if has_no_childs
        delete=  link_to('', page_path(elem), :method=>:delete, :title=>"#{t('page.delete')} #{elem.title}", :confirm=>t('page.delete_confirm'), :class=>'button delete') 
      else
    ...
    def manager_pages_tree!(tree, options= {})
      ...
      unless node
        ...
        res<< content_tag(:div, link_to(node.header, page_path(node), :title=>t('page.zip_of_page') + "#{node.name}"), :class=>"link #{'root' if root}")
    ...
    def pages_tree!(tree, options= {})
      ...
      unless node                                                                                             #Предположим - пришел корень
        ...
        res= content_tag :li, link_to( node.header, page_path(node), :title=>t('page.zip_of_page')+"#{node.name}"), :class=>(root ? 'root' : '')
        child_res= ''
