class QuestionsController < ApplicationController
  ...
  def index
    @question= @admin.questions.new
    @questions = Question.paginate_all_by_user_id_and_state(@admin.id, 'publicated',
  ...
  def box
    @question= @admin.questions.new
    @questions = Question.paginate_all_by_user_id_and_state(@admin.id, ['new_question','seen', 'publicated'],
                                                  :order=>"created_at DESC", #ASC, DESC
                                                  :page => params[:page],
                                                  :per_page=>6
                                                  )
  ...
  def destroy
    @question= Question.find_by_zip(params[:id])
    @question.deleting
    redirect_back_or(question_url(:subdomain=>@admin.subdomain))
  end
  
  def publication
    @question= Question.find_by_zip(params[:id])
    @question.publication
    redirect_to(box_questions_url(:subdomain=>@admin.subdomain))
  end
